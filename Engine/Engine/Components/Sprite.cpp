...
void diji::Sprite::Init()
{
    ...
    if (m_TileSize == 0 || (m_TileCountY + m_TileCountX) == 0 || m_MaxWidth == 0 || m_MaxHeight == 0)
        throw std::invalid_argument("Tile size, tile count, max width, and max height must be non-zero");
...
void diji::Sprite::GenerateMap()
{
    ...
    const sf::Vector2f wallTopLeft = wall.position;
    const sf::Vector2f wallBottomRight = wall.position + wall.size;
            ...
            for (int y = startY; y < endY; ++y)
            {
                for (int x = 0; x < tilesX; ++x)
                {
                    ...
                    else
                    {
                        ...
                        const int maxTerrainY = static_cast<int>(wall.position.y / m_TileSize) - 1;
